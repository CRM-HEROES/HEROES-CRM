...
class PipelineController extends Controller
{
     ...
     */
    public function index(Project $project, Prospect $prospect, Pipeline $pipeline)
    {
        abort_unless($project->id == $pipeline->project_id, 404);

        return $prospect
            ->labels()
            ->whereIn('id', $pipeline->labels()->get(['id'])->pluck('id'))
            ->orderBy('pipeline_label.order', 'desc')
            ->first();
    }
     ...
     */
    public function update(Project $project, Prospect $prospect, Pipeline $pipeline, Label $label)
    {
        // Label category associated to the current project
        abort_unless($project->id == $pipeline->project_id, 404);
        abort_unless($project->id == $label->category->project_id, 404);
        abort_unless($pipeline->labels()->find($label->id), 404);

        $prospect
            ->labels()
            ->detach($pipeline->labels()->get(['id'])->pluck('id')->toArray());
