<?php

namespace App\Utils\Stat\Chart;

use App\Utils\Stat\Chart\StatChart;
use Illuminate\Support\Facades\DB;

class OrdersStepsCount implements StatChart
{

    public function key()
    {
        return "stat.chart.order.step.count";
    }

    public function value($project)
    {
        return cache()->remember(
            $this->key(), 
            3600, 
            function() use ($project) {
                $orders = DB::table('order_step')
                    ->select('step_id', DB::raw('count(*) as orders_count'))
                    ->join('order_steps', 'order_steps.id', '=', 'order_step.step_id')
                    ->where('order_steps.project_id', $project->id)
                    ->groupBy('step_id')
                    ->get();

                $labelValues = DB::table('order_steps')
                    ->whereIn('id', $orders->map(function($v) {
                        return $v->step_id;
                    }))
                    ->get(['id', 'name', 'bgcolor']);

                $orders = $orders->map(function($order) use($labelValues) {
                        $label = $labelValues->where('id', $order->step_id)->first();
    
                        return array_merge((array) $label, [
                            'count' => $order->orders_count
                        ]);
                    });

                $labels = $orders->map(function($order) {
                    return $order['name'];
                });

                $data = $orders->map(function($order) {
                    return $order['count'];
                });

                $backgroundColor = $orders->map(function($order) {
                    return $order['bgcolor'];
                });
    
                return [
                    'labels' => $labels,
                    'datasets' => [[
                        'data' => $data,
                        'backgroundColor' => $backgroundColor
                    ] ]
                ];
            });
    }
}