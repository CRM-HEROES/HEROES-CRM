...
class TotalCommissionsPerUserPerDay implements StatChartStatChart
{
        ...
        return collect($this->statChart->labels)->map(function($label) use($beginDate, $endDate, $items) {
            ...
            for (
                ...
                $count = $items->filter(function($item) use($label, $date) {
                    ...
                    return $item->user_id == $label['id'] && (new \Carbon\Carbon($item->created_at))->lte($date);
