<template>
    <span v-once class="hc-tag hc-interaction-cell-row">
        <i v-if="interaction.source == 'aircall'">
        ...
        <i v-else-if="interaction.source == 'ringover'">
            <svg viewBox="0 0 40 40">
                <path
        ...
        <span class="hc-interaction-cell-row-date" v-text="date"></span>
        <b v-if="interaction.creator">{{ interaction.creator.name }}</b>
    </span>
</template>
...
export default {
    props: {
         ...
         * We need it when we edit the interaction
         */
        prospect: {
            type: Object,
        },
    ...
    computed: {
         ...
         * from the interaction.started_at field
         */
        date() {
