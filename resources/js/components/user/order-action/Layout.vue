...
export default {
    ...
    computed: {
        ...mapGetters(["orderActions", "user", "userOrderActions", "can"]),
        ...
        all: {
            get: function () {
                return (
                    // this.can("all") ||
                    this.userOrderActions.length == this.orderActions.length
                );
            },
            set: async function (value) {
                if (value) {
                    this.orderActions.forEach((f) => {
                        store.commit(ADD_USER_ORDER_ACTION, f);
                    });
                    store.dispatch(BULK_ADD_USER_ORDER_ACTION, {
                        users: [this.user.id],
                        orderActions: this.orderActions.map((f) => f.id),
                    });
                } else {
                    this.orderActions.forEach((f) => {
                        store.commit(REMOVE_USER_ORDER_ACTION, f);
                    });
                    store.dispatch(BULK_REMOVE_USER_ORDER_ACTION, {
                        users: [this.user.id],
                        orderActions: this.orderActions.map((f) => f.id),
                    });
                }
