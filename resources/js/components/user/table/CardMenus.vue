<template>
    <card-menu-list>
        <template v-if="bulkDisabled">
            ...
            />
            <card-menu
                :disabled="fetchingPreviousPage || usersPage <= 1"
            ...
            <card-menu
                :disabled="fetchingNextPage || users.length < usersCount"
...
export default {
    ...
    created() {
        if (deviceType() != "desktop" && this.usersOptions) {
    ...
    methods: {
         ...
         */
        addColumn() {
            store.commit(OPEN_MODAL, "users-table-add-column");
        },
         ...
         */
        async previousPage() {
            store.commit(SET_USERS_PAGE, parseInt(this.usersPage) - 1);
         ...
         */
        toggleUsersOptions() {
            store.commit(TOGGLE_USERS_OPTIONS);
        },
        ...
        async bulkRemove() {
            ...
            try {
                ...
                await store.dispatch(BULK_REMOVE_USER, usersSelected);
            } finally {
    ...
    computed: {
        ...
        selectedUsersProcessed() {
            return (
                this.users.filter(
                    (p) =>
                        this.usersSelected.indexOf(p.id) >= 0 && p.processed_at
                ).length == this.usersSelected.length
            );
        },
        ...
        count: {
            ...
            set: async function (value) {
                store.commit(SET_USERS_COUNT, value);
